...
class RiskModelPipeline:
    ...
    def run(self, df: pd.DataFrame):
        # Parsel 1 — Veri yukleme & hazirlik
    ...
    def _train_and_evaluate_models(self, Xtr, ytr, Xte, yte, Xoot, yoot):
        ...
        for name, (base_mdl, params) in models.items():
            ...
            for tr, va in skf.split(Xtr[self.final_vars_], ytr):
                ...
                sc.append((ks, auc))
            ks_cv = float(np.mean([s[0] for s in sc])) if sc else 0.0
            auc_cv = float(np.mean([s[1] for s in sc])) if sc else 0.5
            ...
            # Calculate train metrics (was missing for display)
            ks_tr, _ = ks_statistic(ytr, p_tr)
            try:
                auc_tr = roc_auc_score(ytr, p_tr)
                gini_tr = gini_from_auc(auc_tr)
            except Exception:
                # Handle edge cases where AUC cannot be calculated
                auc_tr = 0.5
                gini_tr = 0.0
    ...
    def _select_best_model(self):
        ...
        # Select model with highest Gini_OOT (user requested this)
        top=df.sort_values(["Gini_OOT","KS_OOT","AUC_OOT"], ascending=[False, False, False]).iloc[0]
