...
class InputValidator:
        ...
        try:
            ...
            if path.suffix.lower() not in InputValidator.ALLOWED_EXTENSIONS:
                raise ValidationError(
                    f"Invalid file type: {path.suffix}. " f"Allowed types: {InputValidator.ALLOWED_EXTENSIONS}"
                )
            ...
            if file_size_mb > InputValidator.MAX_FILE_SIZE_MB:
                raise ValidationError(
                    f"File too large: {file_size_mb:.2f}MB. " f"Max allowed: {InputValidator.MAX_FILE_SIZE_MB}MB"
                )
    ...
    def validate_dataframe(
        df: pd.DataFrame, required_columns: Optional[List[str]] = None, target_col: Optional[str] = None
    ) -> pd.DataFrame:
        ...
        if len(df.columns) < InputValidator.REQUIRED_COLUMNS_MIN:
            raise ValidationError(f"DataFrame must have at least {InputValidator.REQUIRED_COLUMNS_MIN} columns")
        ...
        if target_col and target_col in df.columns:
            ...
            if not set(unique_values).issubset({0, 1}):
                raise ValidationError(
                    f"Target column '{target_col}' must contain only 0 and 1. " f"Found: {unique_values[:10]}"
                )
            ...
            if len(value_counts) < 2:
                raise ValidationError(
                    f"Target column must have both classes (0 and 1). " f"Found only: {value_counts.index.tolist()}"
                )
        ...
        for col in df.columns:
            for pattern in suspicious_patterns:
                if pattern in str(col).upper():
                    raise ValidationError(
                        f"Suspicious column name detected: '{col}'. " f"Contains pattern: '{pattern}'"
                    )
    ...
    def validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "calibration_method" in config:
            ...
            if config["calibration_method"] not in allowed_methods:
                raise ValidationError(f"calibration_method must be one of {allowed_methods}")

        if "hpo_method" in config:
            allowed_methods = {"random", "grid", "bayesian"}
            if config["hpo_method"] not in allowed_methods:
                raise ValidationError(f"hpo_method must be one of {allowed_methods}")
...
class SecurityLogger:
    ...
    def log_event(self, event_type: str, details: Dict[str, Any]):
        ...
        event = {"timestamp": datetime.datetime.now().isoformat(), "event_type": event_type, "details": details}
    ...
    def log_access(self, file_path: str, user: Optional[str] = None):
        """Log file access."""
        self.log_event("file_access", {"file": str(file_path), "user": user or "unknown"})

    def log_validation_failure(self, error: str, data_hash: Optional[str] = None):
        """Log validation failure."""
        self.log_event("validation_failure", {"error": error, "data_hash": data_hash})
