    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        psi_data = {
            ...
            'Value': [
                ...
                'Stable' if psi_score and psi_score < 0.1 else
                'Some Drift' if psi_score and psi_score < 0.25 else
                'Significant Drift' if psi_score else 'N/A',

                'Good - No shift detected' if psi_score and psi_score < 0.1 else
                'Moderate - Some population shift' if psi_score and psi_score < 0.25 else
                'Poor - Significant drift detected' if psi_score else 'Cannot calculate',

                'PSI < 0.1: Population stable | 0.1 <= PSI < 0.25: Monitor closely | PSI >= 0.25: Model retraining recommended',

                'No action needed' if psi_score and psi_score < 0.1 else
